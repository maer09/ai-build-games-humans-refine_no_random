...
以如下格式在 `tmp/games/<slug>/README.md` 中记录：
...
- [ ] 已在 `README.md` 中记录 `机制 3 + 视觉 2 + 结构 1`
...
**输出**：`tmp/games/<slug>/README.md`（核心机制、控制方式、对象规格、新颖性理由、标签记录、状态变量表、权衡说明）
...
3. 确定由 3-5 种颜色组成的调色板
4. 设计与标签风格一致的反馈效果
5. 在 `VISUAL_DESIGN.md` 中记录反 AI 通用化规则
...
**输出**：`tmp/games/<slug>/VISUAL_DESIGN.md`（概念、调色板、渲染规格、效果设计）
...
**输出**：`tmp/games/<slug>/SOUND_DESIGN.md`（概念、波形调色板、逐事件规格、动态参数）
...
不要将所有文档合并到一个文件中；使用 `README.md` 作为索引页。
`README.md` 必须至少包含以下相对链接：
...
- 在 `README.md` 和 `logs/improvement_report.md` 中记录方案
...
- 排版实现遵循 `.agents/skills/styling-web-game-typography/references/typography-implementation-guide.md`，并更新 `TYPOGRAPHY_DECISION.md`、`THIRD_PARTY_LICENSES.md` 和 `licenses/`
